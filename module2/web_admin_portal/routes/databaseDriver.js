...
DatabaseDriver.prototype.addNewSensor = function(jsonRecords) {
	//insert record
...
DatabaseDriver.prototype.isSensorInRegistry = function(sensorID, callback) {
	this.db.collection(this.registry).find({'sensorid':sensorID}).toArray(function(err, records) {
		if (err) throw err;
		if(records.length>=1)
		{
			console.log("sensor found");
...
DatabaseDriver.prototype.getSensorList = function(callback) {
	/* callback function shall provide and empty array, which is populated during runtime of this function and returned */

	this.db.collection(this.repository).find().toArray( function(err, records) {
		if (err) throw err;
		var result=[];
		records.forEach(function(record) {
			result.push(record.sensorid);
...
DatabaseDriver.prototype.getAllActiveSensors = function(timelimit, callback) {
	/* timelimit must be in miliseconds */
	this.db.collection(this.registry).find().toArray( function(err, records) {
		if (err) throw err;
		var d = new Date();
		var result=[];
		records.forEach(function(record) {
			if(true) {
				var line={sid:record.sensorid, gid: record.gatewayid, ldt:record.lastdowntime, lut: record.lastupdatetime};

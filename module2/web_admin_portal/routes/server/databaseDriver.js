...
DatabaseDriver.prototype.getSensorByLocation = function(mylocation, range, sensortype, callback) {
	...
	this.db.collection(this.repository).find().toArray(function(err, records) {
		...
		records.forEach(function(record) {
			if(record.mode=='M') {
				if(record.lat==mylocation.latitude && record.long==mylocation.longitude)
					result.push(record.sensorid);
			}
			else {
				if(inRange2(mylocation, record.lat, record.long, range))
					result.push(record.sensorid);
			}
		});
		callback(result);
...
function inRange2(mylocation, lat,long, range)
{
	var slat=lat;
	var slong=long; //1
	var mylat=mylocation.latitude;
